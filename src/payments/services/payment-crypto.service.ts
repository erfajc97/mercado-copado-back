...
/**
 * Servicio especializado en pagos por criptomonedas (USDT BEP-20/TRC-20).
 * Responsabilidad única: procesar pagos por transferencia crypto.
 */
@Injectable()
export class PaymentCryptoService {
  ...
  /**
   * Procesa un pago por criptomonedas.
   */
  ...
  /**
   * Procesa depósito crypto para una orden existente (retry de pago).
   */
  private async processExistingOrderDeposit(
    ...
    const transaction = await this.prisma.paymentTransaction.create({
      data: {
        ...
        status: PaymentStatus.pending,
        paymentProvider: PaymentProvider.CRYPTO as any,
        addressId,
      },
    });
  ...
  /**
   * Procesa depósito crypto para una nueva orden (flujo normal desde checkout).
   */
